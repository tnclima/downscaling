...
for(i_ds in ds_variants){
  
  files_ds <- dir_ls(path(path_out, str_c("ds-", i_ds)))
  ...
  dat_inv_ds <- files_ds %>% 
    ...
    str_split_fixed("_", 5) %>% 
    as.data.table()
  setnames(dat_inv_ds, c("variable", "centers", "institute_rcm", "gcm", "experiment"))
  dat_inv_ds[, file1 := files_ds]
  dat_inv_ds_mod <- dat_inv_ds[, .(gcm, institute_rcm, experiment, centers)] %>% unique()
  
  zz <- foreach(
    i_inv = 1:nrow(dat_inv_ds_mod),
    .inorder = F
  ) %dopar% {
    
    dat_inv_ds_mod[i_inv, ] %>% 
      merge(dat_inv_ds) %>% 
      {setNames(.$file1, .$variable)} -> files_rcm
    
    files_obs_orog <- l_file_obs_orog[var_order] %>% unlist
    
    files_out <- path(path_out,
                      str_c("ds-", i_ds, "-ba-mbcn"),
                      dat_inv_ds_mod[i_inv, 
    ...
    dir_create(path_dir(files_out[1]))
    walk(var_order, \(vv){
    ...
    for(i_cell in cells_obs){
      ...
      i_row <- rowFromCell(rss_obs_orog, i_cell)
      i_col <- colFromCell(rss_obs_orog, i_cell)
      ...
      l_vals_rcm <- map(files_rcm, \(x) get_nc_1d(x, i_row, i_col))
      ...
      for(years_train_period in l_years_train_period){
        ...
        for(i_month in 1:12){
          ...
          dat_rcm_fut_window3 <- dat_rcm_fut_window2[, .(date)] %>% 
            ...
            melt(id.vars = c("date"), value.name = "value_mbcn") %>% 

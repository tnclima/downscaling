...
path_out <- "/home/climatedata/downscaling/gcm-bads-qdm-tnaa/"
...
n_cores <- 8 # parallel computation; bottleneck maybe disk access?
...
zz <- foreach(
  ...
  file_out <- path(path_out,
                   i_exp,
                   dat_inv_loop[i_inv, 
                                paste(variable, paste0("model", centers), institute_rcm, 
                                      gcm, sep = "_")],
                   ext = "nc")
  
  dir_create(path(path_out, i_exp))

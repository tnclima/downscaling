...
path_out <- "/home/climatedata/downscaling/validation-cv-reanalysis/data-daily-v3/"
  ...
  dat_coef_out <- foreach(
    ...
    if(i_var == "pr"){
      ...
      if(all(!sf_obs_011_pca$value_wet, na.rm = T)){
        ...
        # intensity
        lm2 <- lm(value ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6, data = sf_obs_011_pca)
      ...
      lm4 <- lm(value ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6, data = sf_obs_011_pca)
...
saveRDS(zz, "data/coef-obs-pcalm.rds")

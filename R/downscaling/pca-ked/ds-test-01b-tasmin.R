...
path_out <- "/home/climatedata/downscaling/pca-ked/ds-test/tasmin_v3/"
match_gridcell <- F # choose gridcells that match elevation?
...
dat_gridcells <- readRDS("data/ked-gridcells-temperature.rds") %>% as.data.table()
...
zz <- foreach(
  ...
  for(i_date in seq_along(dates_loop)){
    ...
    if(match_gridcell){
      ...
      dat_zz <- sf_rcm_i2 %>% 
        st_drop_geometry() %>% 
        merge(dat_gridcells[institute_rcm == i_rcm_name])
      
      sf_rcm_i2_pca <- sf_pca %>% 
    ...
    lm2 <- lm(value ~ PC1, data = sf_rcm_i2_pca)
    lm3 <- lm(value ~ orog + PC2 + PC3 + PC4 + PC5 + PC6, data = sf_rcm_i2_pca)
    lm4 <- lm(value ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6, data = sf_rcm_i2_pca)
    ...
    k1_fit <- krige(value ~ orog, sf_rcm_i2_pca, sf_newdata, v1_fit, debug.level = 0)
    
    v2_sample <- variogram(value ~ PC1, sf_rcm_i2_pca)
    v2_fit <- fit.variogram(v2_sample, vgm("Sph"))
    k2_fit <- krige(value ~ PC1, sf_rcm_i2_pca, sf_newdata, v2_fit, debug.level = 0)
    
    v3_sample <- variogram(value ~ orog + PC2 + PC3 + PC4 + PC5 + PC6, sf_rcm_i2_pca)
    v3_fit <- fit.variogram(v3_sample, vgm("Sph"))
    k3_fit <- krige(value ~ orog + PC2 + PC3 + PC4 + PC5 + PC6, 
                    sf_rcm_i2_pca, sf_newdata, v3_fit, debug.level = 0)
    
    v4_sample <- variogram(value ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6, sf_rcm_i2_pca)
    v4_fit <- fit.variogram(v4_sample, vgm("Sph"))
    k4_fit <- krige(value ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6, 
                    sf_rcm_i2_pca, sf_newdata, v4_fit, debug.level = 0)

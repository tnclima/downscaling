...
for(i_seas in names(seasons)){
  ...
  mat_obs_01 <- mat_obs > th
  
  # pca2 <- prcomp(mat_obs[, 1:100], center = center_pca, scale = scale_pca, rank. = 20)
  # 
  ...
  lpca1 <- logisticPCA(mat_obs_01, k = n_pc, main_effects = main_effects)
  
  dat_pca <- data.table(variable = i_var, season = i_seas, icell = vec_icell, lpca1$PCs)
  setnames(dat_pca, str_c("V", 1:n_pc), str_c("PC", 1:n_pc))
  l_out[[i_seas]] <- dat_pca
  
  # dat_pca_varexp <- summary_prcomp(lpca1, n_pc)
  # l_out_varexp[[i_seas]] <- dat_pca_varexp
...
file_out <- path(path_out, 
                 paste0("pcalog-season-sub", n_sub_dates, 
                        "-", i_var,
                        "-th", th,

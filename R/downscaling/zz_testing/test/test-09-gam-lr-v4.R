...
f_cons <- function(vals_out, ratio = T){
  ...
  if(conserve_signal != "no"){
    ...
    if(conserve_signal == "cell"){
      ...
      vals_out <- rs_ds_scaled[] %>% as.vector
    } 
    
    if(conserve_signal == "largescale_nonzero"){
      if(ratio){
        vals_out[rs_rcm_i2[] < 1] <- 0
        vals_out <- vals_out / mean(vals_out) * mean(rs_rcm_i2[]) 
      } else {
        # vals_out <- vals_out - mean(vals_out) + mean(rs_rcm_i2[]) 
      }
...
conserve_signal <- "largescale_nonzero" # one of "no", largescale", "cell", "largescale_nonzero"
...
## fit models --------------------------------------------------------------
    ...
    lm_orog <- lm(vals_rcm ~ orog, data = dt_rcm[vals_rcm >= 1])
    ...
    vals_out4 <- predict(lm_orog, dt_obs_orog)

...
dat_ba <- readRDS("/home/climatedata/downscaling/validation-cv/rdata-summary-v2/eval-ba-month.rds")
dat_ba[, period20 := ifelse(period20 == "1981-2000", "period1", "period2")]
setnames(dat_ba, "variable", "vv")
dat_ba <- dat_ba[ba %in% c("qm", "qdm")]
...
# eobs
dat_eobs <- readRDS("/home/climatedata/downscaling/validation-cv/rdata-summary-v2/eval-eobs-month.rds")
dat_eobs[, period20 := ifelse(period20 == "1981-2000", "period1", "period2")]
setnames(dat_eobs, "variable", "vv")

dat_eobs1 <- dat_eobs %>% 
  melt(measure.vars = c(str_c("p", pctl*100), "mean_value"),
       variable.factor = F) %>% 
  dcast(... ~ period20)
setnames(dat_eobs1, c("period1", "period2"), str_c("eobs_", c("period1", "period2")))
...
dat_aux_rcm <- dat_aux_rcm[, .(centers, icell, orog_rcm = orog)] 

# 1mod example -----------------------------------------------------------------

i_center <- 1

dat_1mod <- dat_ba[centers == i_center]
...
dat_plot <- dat1[month %in% c(1, 4, 7, 10),
  ...
  rbind(dat_rcm1[month %in% c(1, 4, 7, 10), 
                 .(icell, month, ba = "rcm-raw", variable, vv, 
                   period1 = rcm_period1, period2 = rcm_period2)]) %>% 
  rbind(dat_eobs1[month %in% c(1, 4, 7, 10),
                  .(icell, month, ba = "eobs", variable, vv, 
                    period1 = eobs_period1, period2 = eobs_period2)]) %>% 
  merge(dat_aux, by = "icell")

dat_plot[, ba_fct := fct_relevel(ba, "rcm-raw", "eobs")]
...
dat_plot[variable == "p95" & vv == "tasmax"] %>% 
  ggplot(aes(lon, lat, fill = period2 - period1))+
  geom_raster()+
...
dat_plot2 <- dat1[variable == "p95" & vv == "tasmax" & month %in% c(1, 4, 7, 10)] %>% 
  merge(dat_rcm1, by = c("icell", "month", "vv", "variable", "centers")) %>% 
  merge(dat_eobs1) %>% 
  merge(dat_aux, by = "icell")
...
dat_plot3 <- rbind(
  dat_plot[variable == "p95" & vv == "tasmax", 
           .(lon, lat, month_fct, ba_fct, rr = period2 - period1)],
  dat_plot2[, .(lon, lat, month_fct, ba_fct,
                rr = period2 - period1 - 2*(rcm_period2 - rcm_period1) + (eobs_period2 - eobs_period1))]
)

dat_plot3 %>% 
  ggplot(aes(lon, lat, fill = rr))+
  geom_raster()+
...
dat_eobs1_subrep <- lapply(1:6, \(x) cbind(
  dat_eobs1[month %in% c(1, 4, 7, 10) & vv %in% c("tasmin", "tasmax")],
  centers = x
)) %>% rbindlist()
...
dat_plot1 <- dat_allmod_longsub[,
  ...
  rbind(dat_rcm1[month %in% c(1, 4, 7, 10) & vv %in% c("tasmin", "tasmax"), 
                 .(icell, month, centers, ba = "rcm-raw", variable, vv, 
                   period1 = rcm_period1, period2 = rcm_period2)]) %>% 
  rbind(dat_eobs1_subrep[,
                         .(icell, month, centers, ba = "eobs", variable, vv, 
                           period1 = eobs_period1, period2 = eobs_period2)]) %>% 
  merge(dat_aux, by = "icell")

dat_plot1[, ba_fct := fct_relevel(ba, "rcm-raw", "eobs")]


dat_plot2 <- dat_allmod_longsub %>% 
  merge(dat_rcm1, by = c("icell", "month", "vv", "variable", "centers")) %>% 
  merge(dat_eobs1_subrep) %>% 
  merge(dat_aux, by = "icell")

dat_plot2[, ba_fct := str_c(ba, "_adj")]

dat_plot3 <- rbind(
  dat_plot1[, .(icell, lon, lat, month, ba_fct, variable, vv, centers,
                rr = period2 - period1)],
  dat_plot2[, .(icell, lon, lat, month, ba_fct, variable, vv, centers,
                rr = period2 - period1 - 2*(rcm_period2 - rcm_period1) + (eobs_period2 - eobs_period1))]
)
...
for(i in 1:nrow(dat_loop)){
  ...
  fn <- path("fig/validation-eval/trend-preserv-ba/", 
...
gg1 <- dat_plot3[ba_fct %in% c("qdm_adj", "qm_adj") & vv == "tasmax"] %>% 
  ...
  ylab("count (tot = ~400 cells * 4 months * 6 models)")+
  ggtitle("tasmax")

ggsave("fig/validation-eval/trend-preserv-summary/tasmax-hist_baonly.png", 
...
ggsave("fig/validation-eval/trend-preserv-summary/tasmin-hist_baonly.png", 
       gg1, width = 12, height = 4)
...
dat_sub1 %>% 
  merge(dat_aux_rcm) %>% 
  merge(dat_aux, by = "icell") %>% 
  ggplot(aes(elevation - orog_rcm, rr))+
...
# -> some dependency with orog_rcm (and orog, since similar, but not difference)
...
dat_plot3 <- rbind(
  ...
  dat_plot2[, .(icell, lon, lat, month, ba_fct, variable, vv, centers,
                rr = (period2 - period1)/period1 -
                  ...
                  (eobs_period1 - eobs_period2)/eobs_period2)]
)
dat_plot3[, month_fct := mitmatmisc::month_fct(month)]
...
# 
# 
dat_plot2[vv == "pr" & variable == "p50" & centers == 1] %>%
  # ggplot(aes(x, y, fill = (period2 - period1)/period1))+
  ...
  # ggplot(aes(lon, lat, fill = (period2 - period1)/period1 - 2*((rcm_period2 - rcm_period1)/rcm_period1) - (eobs_period1 - eobs_period2)/eobs_period2 ))+
  ggplot(aes(lon, lat, fill = period2/period1 - (rcm_period2/rcm_period1)*(eobs_period1/eobs_period2) ))+

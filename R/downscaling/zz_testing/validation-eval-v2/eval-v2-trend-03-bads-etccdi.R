...
etccdi <- c("tr", "su", "id", "fd", "rx1day", "r20mm", "wd", "sdii", "cdd")
# etccdi_temp <- c("tr", "su", "id", "fd", "rx1day", "r20mm", "wd", "sdii", "cdd")
...
# crespi
dat_crespi <- readRDS("/home/climatedata/downscaling/validation-cv/rdata-summary-v2/eval-crespi-etccdi.rds")
setnames(dat_crespi, "variable", "vv")
dat_crespi[, period20 := ifelse(period20 == "1981-2000", "period1", "period2")]

dat_crespi1 <- dat_crespi %>% 
  dcast(... ~ period20)
setnames(dat_crespi1, c("period1", "period2"), str_c("crespi_", c("period1", "period2")))


# rcm-raw
dat_rcm <- readRDS("/home/climatedata/downscaling/validation-cv/rdata-summary-v2/eval-rcm-raw-etccdi.rds")
setnames(dat_rcm, "variable", "vv")
dat_rcm[, period20 := ifelse(period20 == "1981-2000", "period1", "period2")]
...
dat_files_allmod <- dat_files[freq == "etccdi" & ba %in% c("qm", "qdm")]
...
dat_plot1 <- dat_allmod_longsub[,
                                .(icell, centers, ba, vv, period1, period2)] %>% 
  rbind(dat_rcm1_1km[, 
                     .(icell, centers, ba = "rcm-raw", vv, 
                       period1 = rcm_period1, period2 = rcm_period2)]) %>% 
  rbind(dat_crespi1_subrep[,
                           .(icell, centers, ba = "crespi", vv, 
...
dat_plot2 <- dat_allmod_longsub %>% 
  merge(dat_rcm1_1km, by = c("icell", "vv", "centers")) %>% 
...
dat_plot3 <- rbind(
  dat_plot1[, .(icell, x, y, ba_fct, vv, centers,
                rr = period2 - period1)],
  dat_plot2[, .(icell, x, y, ba_fct, vv, centers,
...
dat_loop <- dat_plot3[, .(vv)] %>% unique

for(i in 1:nrow(dat_loop)){
  
  dat_i <- dat_plot3 %>% merge(dat_loop[i])
  tit <- dat_loop[i, str_c(vv)]
  fn <- path("fig/validation-eval/trend-preserv-etccdi/", 
             dat_loop[i, str_c(vv)],
             ext = "png")
  ...
  gg <- dat_i %>% 
    ...
    facet_grid(centers ~ ba_fct)+
    coord_fixed()+
    theme_bw()+
    xlab(NULL)+ylab(NULL)+
    ggtitle(tit)
  
  ggsave(fn, gg, width = 12, height = 8)

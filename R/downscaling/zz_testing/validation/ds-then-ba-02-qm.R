...
for(i_ds in ds_variants){
  ...
  zz <- foreach(
    ...
    i_file_ds_split <- i_file_ds %>% 
      path_file() %>% 
      str_split_1("_")
    
    i_rcm_name <- i_file_ds_split[3]
    i_var <- i_file_ds_split[1]
    ...
    for(i_cell in cells_obs){
      ...
      for(years_train_period in l_years_train_period){
        ...
        if(detrend & i_var != "pr"){
          ...
          dat_rcm_hist_lm <- dat_rcm_hist[, broom::tidy(lm(value ~ year0)), .(month)]
          dat_rcm_hist <- dat_rcm_hist_lm[term == "year0", 
                                          .(month, slope_year = estimate)] %>% 
            merge(dat_rcm_hist)
        ...
        for(i_month in 1:12){
          ...
          if(detrend & i_var != "pr"){
            ...
            # detrend future
            dat_rcm_fut_window_lm <- dat_rcm_fut_window[, broom::tidy(lm(value ~ year0)), .(month)]
          ...
          dat_rcm_fut_window[month == i_month, 

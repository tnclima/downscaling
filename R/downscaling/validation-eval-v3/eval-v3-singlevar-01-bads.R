...
# rcm-raw
dat_rcm_month <- readRDS("/home/climatedata/downscaling/validation-cv/rdata-summary-v3/eval-rcm-raw-month.rds")
setnames(dat_rcm_month, "variable", "vv")
dat_rcm_month[, period20 := ifelse(period20 == "odd", "period1", "period2")]
...
dat_files_allmod <- dat_files[freq == "month"]
...
dat_plot1 <- dat_allmod_avg %>% 
  ...
  merge(dat_aux)
dat_plot1[, month_fct := mitmatmisc::month_fct(month, 1)]

dat_plot2 <- dat_plot1 %>% 
  dcast(... ~ ba, value.var = "value")
# abs
dat_plot2[vv %in% c("tasmin", "tasmax"), qm_crespi := qm - crespi]
dat_plot2[vv %in% c("tasmin", "tasmax"), qdm_crespi := qdm - crespi]
dat_plot2[vv %in% c("tasmin", "tasmax"), mbcn_crespi := mbcn - crespi]
# dat_plot2[vv %in% c("tasmin", "tasmax"), mbcn_qdm := mbcn - qdm]
# rel
dat_plot2[!vv %in% c("tasmin", "tasmax"), qm_crespi := (qm - crespi)/crespi]
dat_plot2[!vv %in% c("tasmin", "tasmax"), qdm_crespi := (qdm - crespi)/crespi]
dat_plot2[!vv %in% c("tasmin", "tasmax"), mbcn_crespi := (mbcn - crespi)/crespi]
# dat_plot2[!vv %in% c("tasmin", "tasmax"), mbcn_qdm := (mbcn - qdm)/qdm]

dat_plot3 <- melt(dat_plot2, 
                  id.vars = c("icell", "month", "variable", "vv", "centers", "x", "y", "orog", "month_fct"),
                  measure.vars = c("qm_crespi", "qdm_crespi", "mbcn_crespi"),
                  variable.name = "ff_diff")

# loop ---------------------------------------------------------------


dat_loop <- dat_plot3[, .(vv, variable, centers)] %>% unique
...
for(i in 1:nrow(dat_loop)){
  ...
  fn <- path("fig/validation-eval/singlevaroddeven/", 
  ...
  if(dat_loop[i, vv] == "pr"){
    gg2 <- gg2 + scale_fill_scico("", palette = "vik", midpoint = 0,labels = scales::label_percent())
  } else {
    gg2 <- gg2 + scale_fill_scico("", palette = "vik", midpoint = 0)
  }
  
  gg <- gg1/gg2
  
  ggsave(fn, gg, width = 18, height = 8)
...
dat_summ1 <- dat_plot1[, .(value = mean(value)), .(vv, month, month_fct, variable, centers, ba)]
dat_summ3 <- dat_plot3[, .(value = mean(value)), .(vv, month, month_fct, variable, centers, ff_diff)]

for(i_var in c("tasmax", "tasmin", "pr")){
  gg <- dat_summ1[vv == i_var] %>% 
    ggplot(aes(month_fct, value, colour = ba))+
    geom_line(aes(group = paste0(ba, centers)))+
    facet_grid(variable ~ centers, scale = "free_y")+
    theme_bw()+
    xlab(NULL)+ylab(i_var)
  
  ggsave(str_c("fig/validation-eval/singlevar-summary/", i_var, "-month-raw_oddeven.png"),
         gg, width = 16, height = 8)
  
  
  gg <- dat_summ3[vv == i_var] %>% 
    ...
    theme_bw()+
    xlab(NULL)+
    ylab(str_c("Difference ", i_var))
  
  if(i_var == "pr") gg <- gg + scale_y_continuous(labels = scales::label_percent())
  
  ggsave(str_c("fig/validation-eval/singlevar-summary/", i_var, "-month-diff_oddeven.png"),
         gg, width = 16, height = 6)

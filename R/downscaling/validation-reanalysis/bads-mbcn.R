...
path_out <- "/home/climatedata/downscaling/validation-cv-reanalysis/data-daily/bads-mbcn/"
...
zz <- foreach(
  ...
  for(i_cell in cells_obs){
    ...
    l_vals_rcm <- map(l_mat_rcm, \(mat_rcm) mat_rcm[i_cell_rcm, ])
        ...
        # for(i_decade in all_decades){
          ...
          walk(var_order_ba, \(i_var){
            dat_rcm[month == i_month & 
                      !year %in% years_train_period &
                      variable == i_var, 
                    value_ba := dat_rcm_fut_window3[month == i_month & variable == i_var,
                                                    value_mbcn]]

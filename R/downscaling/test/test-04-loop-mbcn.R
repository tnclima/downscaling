...
path_out <- "/home/climatedata/downscaling/zz_temp/loop-test-mbcn/"
...
for(i_inv in 1:nrow(dat_inv_loop_mod)){
  ...
  if(all(file_exists(files_out))) next
  ...
  for(i_cell in cells_obs){
    ...
    if(detrend){
      ...
      dat_obs2_lm <- dat_obs2[variable %in% c("tasmax", "tasmin"), 
                              broom::tidy(lm(value ~ year0)), 
                              .(month, variable)]
      dat_obs2 <- dat_obs2_lm[term == "year0", .(month, variable, slope_year = estimate)] %>% 
        merge(dat_obs2, all.y = T)
      dat_obs2[variable %in% c("tasmax", "tasmin"), value_detrended := value - slope_year*year0]
      dat_obs2[!variable %in% c("tasmax", "tasmin"), value_detrended := value]
      
      # ggplot(dat_obs2, aes(year, value))+
      #   geom_point()+
    ...
    dat_rcm_hist <- dat_rcm[year >= years_train_period[1] & 
      ...
      melt(id.vars = c("date", "year", "year0", "month", "variable"),
    ...
    for(i_month in 1:12){
      ...
      vals_train_obs <- dat_obs2[month %in% i_month_window & variable %in% var_order_ba] %>%
      ...
      for(i_decade in all_decades){
        ...
        dat_rcm_fut_window <- dat_rcm[month %in% i_month_window  & decade %in% i_decade_window] %>% 
          melt(id.vars = c("date", "year", "year0", "decade", "month", "variable"), 
               measure.vars = patterns("^V"),
               variable.name = "cell")
        if(detrend){
        ...
        walk(var_order_ba, \(i_var){
          dat_rcm[month == i_month & decade == i_decade & variable == i_var, 
                  value_ba := dat_rcm_fut_window3[cell == "V1" & month == i_month & decade == i_decade & variable == i_var,
                                                  value_mbcn]]
